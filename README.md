...
| `ONEC_AI_INCLUDE_LIMITATIONS` | нет | `true` |
...
Успешный ответ каждого инструмента содержит секции `Ответ`, `Ограничения
инструмента` и `Требуемая проверка`. Ограничения формируются MCP-сервером, а не
